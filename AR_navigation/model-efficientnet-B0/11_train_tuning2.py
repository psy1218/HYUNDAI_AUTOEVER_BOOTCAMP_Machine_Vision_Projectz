...
train_transform = transforms.Compose([
    ...
    transforms.RandomRotation(2),   # 5 -> 3으로 약하게 조정 -- 
    transforms.ToTensor(),
...
def compute_class_weights_from_dataset(dataset, device):
    """
    ImageFolder dataset의 targets를 이용해 클래스별 weight 계산
    샘플 수가 적은 클래스일수록 더 큰 weight를 부여
    """
    targets = dataset.targets  # 각 이미지의 클래스 인덱스 리스트
...
#    print(f"Weight decay         : {WEIGHT_DECAY}")
    ...
    # class weight 계산
    class_weights, class_counts = compute_class_weights_from_dataset(train_dataset, device)

    print_section("Class Weight Info")
    for i, class_name in enumerate(class_names):
        print(f"{class_name:>4} | count: {class_counts[i]:>4} | weight: {class_weights[i].item():.4f}")
