...
SAVE_PATH = "/home/psy1218/projects/1_pro/efficientnet_b0_best.pth"
...
def evaluate(model, loader, criterion, device, mode="Val"):
    ...
    dataset_samples = loader.dataset.samples  # (path, class_idx)
    current_index = 0
...
def build_prediction_dataframe(y_true, y_pred, file_paths, class_names):
    ...
    for true_idx, pred_idx, file_path in zip(y_true, y_pred, file_paths):
        true_class = class_names[true_idx]   # 예: 1_W
        pred_class = class_names[pred_idx]   # 예: 3_W
...
def save_prediction_csvs(df, save_dir="/home/psy1218/projects/1_pro"):
    ...
    all_csv_path = os.path.join(save_dir, "test_predictions_all.csv")
    wrong_csv_path = os.path.join(save_dir, "test_predictions_wrong_only.csv")
...
def compute_detailed_metrics(y_true, y_pred, class_names):
    ...
    # 구역 맞은 경우의 방향 accuracy
    matched_zone_mask = [tz == pz for tz, pz in zip(true_zones, pred_zones)]

    true_dirs_when_zone_correct = [
        td for td, ok in zip(true_dirs, matched_zone_mask) if ok
    ]
    pred_dirs_when_zone_correct = [
        pd for pd, ok in zip(pred_dirs, matched_zone_mask) if ok
    ]
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        val_loss, val_acc, _, _, val_time = evaluate(
    ...
    print(cm)
    plot_confusion_matrix(cm, class_names, title="EfficientNet-B0 Test Confusion Matrix")
